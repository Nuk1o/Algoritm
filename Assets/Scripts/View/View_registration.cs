...
public class View_registration : MonoBehaviour //Регистрация пользователя
{
    ...
    public void reg_click()
    {
        ...
        try
        {
            ...
            var encrypted = MV_pass_encryption.Encrypt(Encoding.UTF8.GetBytes(login_str), password_str, salt, iv);//зашифровал
                                                                                                                  //Debug.Log($"Пароль: {Convert.ToBase64String(encrypted)}");

            var decrypted = MV_pass_encryption.Decrypt(encrypted, password_str, salt, iv);//Расшифровал
